...
class climate_stripes:
    ...
    def __init__(self, stnid, begin_date, end_date, mytoken):
        ...
        **Example:**
        ...
        self.token = mytoken
        # assign an empty attribute, to store the daily mean temperature and
        # anomalies
        ...
        **Returns**
        ...
        if self.ed.year == self.bd.year:
            ...
            r = requests.get(
                ...
                'datasetid=GHCND&datatypeid=TMIN&datatypeid=TMAX' +
                '&stationid=' +
                self.stnid +
                '&startdate=' +
                year +
            ...
            d = json.loads(r.text)
            # get all items in the response which are max&min temperature
            # readings
            maxtemps = [item for item in d['results']
                        if item['datatype'] == 'TMAX']
            mintemps = [item for item in d['results']
                        if item['datatype'] == 'TMIN']
            # get the date field from all average temperature readings
            dates_maxtemp += [item['date'] for item in maxtemps]
            dates_mintemp += [item['date'] for item in mintemps]
            # get the actual average temperature from all average temperature
            # readings
            ...
            for year in range(self.bd.year, self.bd.year + 1):
                ...
                dates_maxtemp += [item['date'] for item in maxtemps]
                dates_mintemp += [item['date'] for item in mintemps]
                # get the actual average temperature from all average
        ...
        # populate date and min and max temperature fields (convert string date
        # to datetime)
        df_temp_min['date'] = [
            datetime.strptime(
                d, "%Y-%m-%dT%H:%M:%S") for d in dates_mintemp]
        df_temp_min['minTemp'] = [float(v) / 10.0 for v in min_temps]

        df_temp_max['date'] = [
            datetime.strptime(
                d, "%Y-%m-%dT%H:%M:%S") for d in dates_maxtemp]
        df_temp_max['maxTemp'] = [float(v) / 10.0 for v in max_temps]

        # drop row if one of the temp is missing for a specific day
        newdf_all = pd.merge_asof(
            df_temp_min, df_temp_max, on='date', by='date')
    ...
    def _makeanomaly(self):
        ...
        **Returns**
        ...
        """
        DF = self._data()
        # Remove annual cycle from daily data, i.e. remove the multi-year mean
        # of each day
        DF['anom'] = DF['average'].groupby(
            DF.index.dayofyear).transform(
            lambda x: (
                x - x.mean()))
        return DF
